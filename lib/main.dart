...
class _StartPageState extends State<StartPage> {
  ...
  Future<Map<String, dynamic>> getTotalWeightForPet(String petName) async {
    ...
    final String todayString = DateFormat('yyyyMMdd').format(selectedDate);
    ...
    for (var petDoc in petQuery.docs) {
      final docId = petDoc.id; // This is the document ID

      // Get the document in the 'record' collection that matches the current date
      final recordDoc = await firestore.collection('pet')
          .doc(docId)
          .collection('record')
          .doc(todayString)
          .get();
      ...
      if (recordDoc.exists) {
        final Map<String, dynamic> todayData = recordDoc.data()?[todayString] as Map<String, dynamic>? ?? {};
        final Map<String, dynamic> feedingAmount = todayData['배식량'] as Map<String, dynamic>? ?? {};
          ...
          Container(
            ...
            child: PageView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                double smallBoxSize_w = iconHeight * 0.76; // 반려동물 3박스 사이즈
                double smallBoxSize_h = iconHeight * 0.55;
